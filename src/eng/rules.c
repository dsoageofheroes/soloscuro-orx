...
static void set_psp(sol_entity_t *pc) {
    ...
    if (sol_entity_has_class(pc, REAL_CLASS_PSIONICIST) == EXIT_SUCCESS) {
        int psi_level = 0;
...
static void do_level_up(sol_entity_t *pc, const uint32_t class_idx, const uint32_t class) {
    ...
    int num_classes = 0, status = EXIT_FAILURE;
...
static int calc_starting_exp(sol_entity_t *pc) {
    ...
    for (int i = 0; i < 3; i++) {
        if (pc->class[i].class > 0 ) { num_classes++; }
    }

    starting_level = (num_classes > 1) ? 2 : 3;

    for (int i = 0; i < 3; i++) {
        sol_dnd2e_next_level_exp(pc->class[i].class, starting_level - 1, &next_level);
        most_exp = most_exp > next_level ? most_exp : next_level;
    }
    //printf("%d %d %d\n", most_exp, starting_level , num_classes);
...
extern int sol_dnd2e_loop_creation_stats(sol_entity_t *pc) {
    if (!pc) {return EXIT_FAILURE; }
    if (pc->race < 0 || pc->race > RACE_THRIKREEN) { return EXIT_FAILURE; }
...
extern int sol_dnd2e_character_is_valid(const sol_entity_t *pc) {
    if (!sol_dnd2e_stats_valid(&pc->stats)) { return EXIT_FAILURE; }
    if (sol_dnd2e_is_class_allowed(pc->race, pc->class) != EXIT_SUCCESS) { return EXIT_FAILURE; }
    if (pc->gender != GENDER_MALE && pc->gender != GENDER_FEMALE) { return EXIT_FAILURE; }
    if (pc->alignment < LAWFUL_GOOD || pc->alignment > CHAOTIC_EVIL) { return EXIT_FAILURE; }
    if (pc->class[0].level < 1) { return EXIT_FAILURE; }
    if (pc->class[1].class > -1 && pc->class[1].level < 1) { return EXIT_FAILURE; }
    if (pc->class[2].class > -1 && pc->class[2].level < 1) { return EXIT_FAILURE; }
    if (pc->stats.magic_resistance < 0 || pc->stats.magic_resistance > 100) { return EXIT_FAILURE; }
    if (pc->allegiance != 1) { return EXIT_FAILURE; }
    if (pc->size < 0) { return EXIT_FAILURE; }
    // Not checked:
    // pc->id
    // pc->data1
    // pc->legalClass
    ...
    // pc->psi_group
    // pc->pallette
    return EXIT_SUCCESS;// passed the checks.
}
...
extern int sol_dnd2e_psin_is_valid(ds_character_t *pc, sol_psin_t *psi) {
    ...
    if (is_psionicist) { return num_psionics == 3 ? EXIT_SUCCESS : EXIT_FAILURE; }

    return num_psionics == 1 ? EXIT_SUCCESS : EXIT_FAILURE;
}
...
extern int sol_dnd2e_can_melee_again(sol_entity_t *source, const int attack_num, const int round) {
    ...
    return (next_attack != -1 && next_attack != SLOT_END) ? EXIT_SUCCESS : SOL_NO_MELEE_LEFT;
}
*/
...
extern sol_attack_t sol_dnd2e_range_attack(sol_entity_t *source, sol_entity_t *target, const int round) {
    ...
    sol_entity_distance(source, target, &dist);
