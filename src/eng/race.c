...
extern const int8_t sol_dnd2e_race_get_stats(int race, int stat, int max) {
    return racial_stats[race][stat][max];
}

extern int sol_dnd2e_race_apply_mods(sol_entity_t *pc) {
    if (!pc) { return EXIT_FAILURE; }
    if (pc->race < RACE_HUMAN || pc->race > RACE_THRIKREEN) { return EXIT_FAILURE; }
...
extern int sol_dnd2e_race_apply_initial_stats(sol_entity_t *pc) {
    if (!pc) { return EXIT_FAILURE; }
